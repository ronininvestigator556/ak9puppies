...
for puppy_dir in sorted(PUPPIES_BASE.iterdir()):
    if not puppy_dir.is_dir():
        continue
    name = puppy_dir.name
    print(f"\n--- {name} ---")
    ...
    for f in files:
        ...
        # Descriptive already-named files like aang-hero-1.webp.jpg
        if fname.endswith('.webp.jpg'):
            clean = fname[:-9]  # strip .webp.jpg
            ...
            dst = puppy_dir / f"{name}-{stage}-{stage_counters[stage]:03d}.webp"
        else:
            dst = puppy_dir / f"{name}-{generic_counter:03d}.webp"
            generic_counter += 1
